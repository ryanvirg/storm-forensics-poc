...
export function drawStormMap(canvas, {grid = {}, values = [], centroids = [], frame = 0, max = 20, showTrack = true, showGrid = true, geography = null, basemapImage = null, showBoundary = true, rainOpacity = .58, basemapMode = 'imagery'} = {}) {
  if (!canvas) return;
  ...
  if (showTrack && centroids.length) {
    ...
    if (active) {
      ...
      const label = 'Storm centroid', textWidth = ctx.measureText(label).width;
  ...
  canvas.setAttribute('aria-label',`${reference} Frame ${frame+1}. Rainfall ranges from 0 to ${max}. ${showTrack ? 'The white dashed line shows the storm centroid track.' : ''}`);
}
